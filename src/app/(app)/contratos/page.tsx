...
export default async function ContratosPage() {
  ...
  const { data: contratos, error } = await supabase
    .from("contratos")
    .select(
      "*, clientes(nombre, apellido, tipo_persona, razon_social), contrato_propiedades(propiedades(direccion, proyectos(nombre)))"
    )
            ...
            {contratos?.map((c) => (
              <tr key={c.id}>
                ...
                <td className="px-4 py-2 font-medium text-slate-900">
                  {c.clientes ? nombreCliente(c.clientes) : "-"}
                </td>
                <td className="px-4 py-2 text-slate-600">
                  {formatPropiedadesLabel(
                    (c.contrato_propiedades ?? []).map(
