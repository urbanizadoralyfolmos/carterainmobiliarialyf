...
export default async function ContratosPage() {
  ...
  const { data: contratos, error } = await supabase
    .from("contratos")
    .select("*, clientes(nombre, apellido), propiedades(direccion, proyectos(nombre))")
            ...
            {contratos?.map((c) => (
              <tr key={c.id}>
                ...
                <td className="px-4 py-2 text-slate-600">
                  {c.propiedades?.proyectos?.nombre
                    ? `${c.propiedades.proyectos.nombre} - ${c.propiedades.direccion}`
