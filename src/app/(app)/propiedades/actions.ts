...
export async function eliminarPropiedades(formData: FormData) {
  ...
  const redirectTo = String(formData.get("redirect_to") ?? "/propiedades");
  ...
  // Se borra una por una (no en un solo `.in()`) para que, si alguna tiene
  // un contrato vinculado (delete restrict), las demás igual se eliminen
  // en vez de que falle todo el lote.
  for (const id of ids) {
  ...
  if (bloqueadas > 0) {
    const mensaje =
      eliminadas > 0
        ? `Se eliminaron ${eliminadas} propiedad(es). ${bloqueadas} no se pudieron eliminar porque tienen un contrato vinculado.`
        : `No se pudo eliminar ninguna: ${bloqueadas} propiedad(es) tienen un contrato vinculado. Eliminá primero el contrato correspondiente.`;
    const separador = redirectTo.includes("?") ? "&" : "?";
    redirect(`${redirectTo}${separador}error=${encodeURIComponent(mensaje)}`);
  }
