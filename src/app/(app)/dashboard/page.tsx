...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from("contratos").select("id, monto_cuota, moneda").eq("estado", "activo"),
    supabase
      .from("cuotas")
      .select(
  ...
  const carteraMensual = (contratosActivos ?? []).reduce(
    (acc, c) => acc + (c.monto_cuota ?? 0),
    0
  );
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card
          label="Cartera mensual (contratos activos)"
          value={formatMoney(carteraMensual)}
          sub={`${contratosActivos?.length ?? 0} contratos activos`}
        />
        <Card
